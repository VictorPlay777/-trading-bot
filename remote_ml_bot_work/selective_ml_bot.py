...
class SelectiveMLBot:
    ...
    def _create_state(self, sig, qty: Decimal):
        ...
        logger.info(
            ...
            f"sl={st.stop_loss_price:.6f} tp1={st.take_profit_levels['tp1']:.6f} "
    ...
    def _sync_state_from_exchange(self, positions):
        ...
        for p in positions:
            ...
            if sym not in self.position_states:
                ...
                self._create_state(sig, sz)
                self.position_states[sym].exit_state = "open"
            else:
                st = self.position_states[sym]
                prev_rem = self._remaining_qty(st)
                if sz < prev_rem:
                    st.qty_closed += (prev_rem - sz)
                st.updated_ts = time.time()
    ...
    def _monitor_positions(self):
        ...
        for sym, st in list(self.position_states.items()):
            ...
            st.exit_fees_estimate = abs(notional) * float(self.cfg.get("taker_fee", 0.0006))
            unreal = (price - st.entry_price) * float(rem) if st.side == "long" else (st.entry_price - price) * float(rem)
            net_unreal = unreal - st.entry_fees - st.exit_fees_estimate - st.funding_estimate
            ...
            logger.info(
                f"[POSITION] {sym} side={st.side} price={price:.6f} rem={self._remaining_qty(st)} "
                f"net_unreal={net_unreal:.6f} state={st.exit_state}"
            )
    ...
    def _entry_qty(self, sig, size_mult: float):
        ...
        if mode == "max_exchange_qty":
            ...
            max_mkt_qty = rules.get("max_mkt_qty")
            if max_mkt_qty is None:
                return Decimal("0")
            # Apply soft liquidity penalties, but never exceed exchange max.
            q = Decimal(str(max_mkt_qty)) * Decimal(str(size_mult))
            if q > Decimal(str(max_mkt_qty)):
                q = Decimal(str(max_mkt_qty))
            return q
