...
class Exchange:
    def __init__(self, cfg):
        ...
        self._min_request_interval_sec = 0.08
    ...
    def _request(self, method: str, endpoint: str, params: dict = None, auth: bool = True) -> dict:
        ...
        if dt < self._min_request_interval_sec:
            time.sleep(self._min_request_interval_sec - dt)
        ...
        try:
            ...
            # Return business errors to caller; caller should decide retry/skip.
            if data.get("retCode") != 0:
                self.logger.error(
                    f"[API ERROR] endpoint={endpoint} retCode={data.get('retCode')} retMsg={data.get('retMsg')}"
                )
                return data
            ...
            self.logger.error(f"[NETWORK ERROR] endpoint={endpoint} error={e}")
            return {"retCode": -1, "retMsg": f"Request failed: {e}", "result": {}}
    ...
    def normalize_qty(self, symbol: str, qty, price: float = None, qty_in_notional: bool = False, is_market: bool = False):
        ...
        try:
            ...
            self.logger.info(
                ...
                f"step={step} min_qty={min_q} max_qty={max_q} price={price} "
                f"qty_in_notional={qty_in_notional} is_market={is_market}"
            )
    ...
    def market_buy_symbol(self, symbol: str, qty):
        ...
        nqty = self.normalize_qty(symbol, qty, is_market=True)
        if nqty <= 0:
