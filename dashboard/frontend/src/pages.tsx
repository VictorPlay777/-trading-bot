  ...
  return (
    <>
      <PageHeader eyebrow="OPERATIONS" title="Dashboard">
        <div className="header-summary"><StatusBadge status={status} /><span className={clsx('chip', status.bybit.connected ? 'success' : 'danger')}>{status.bybit.connected ? '● Bybit OK' : '● CONNECTION ERROR'}</span><span className="chip">Heartbeat {status.state === 'STOPPED' ? '—' : fmtAge(status.heartbeat_age_sec)}</span><span className="chip">Uptime {status.state === 'STOPPED' ? '—' : fmtDuration(status.uptime_sec)}</span>{status.strategy_id && <span className="chip">{status.strategy_id}</span>}</div>
      </PageHeader>
      ...
      <div className="kpi-grid">
        ...
        <KpiCard label="Drawdown" value={fmtUsd(summary.drawdown.usd, false)} sub={summary.drawdown_source === 'equity_snapshots' ? `${summary.drawdown.pct == null ? '—' : fmtPct(summary.drawdown.pct)} from 90d equity peak` : summary.drawdown_source === 'trades' ? 'from cumulative PnL peak (no equity history)' : 'no equity history yet'} tone="loss" />
