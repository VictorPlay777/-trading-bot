...
import { fmtAge, fmtDuration, fmtPct, fmtPrice, fmtTs, fmtUsd, numberValue, toneFor } from './format'
  ...
  return (
    <div className="login-screen">
      <form className="login-card" onSubmit={(event) => void submit(event)}>
        ...
        <h1>Welcome back</h1>
        <p className="muted">Sign in to monitor and control the trading process.</p>
  ...
  return (
    <>
      <div className="control-grid">
        <button className="button primary" disabled={!stopped || Boolean(status.control.emergency_stop)} onClick={() => void action('/api/bot/start', undefined, 'Bot start requested')}>▶ Start</button>
        <button className="button warning" disabled={!running} onClick={() => void action(status.control.paused ? '/api/bot/resume' : '/api/bot/pause', undefined, status.control.paused ? 'Bot resumed' : 'Bot paused')}>{status.control.paused ? '▶ Resume' : 'Ⅱ Pause'}</button>
        <button className="button subtle" disabled={!running} onClick={() => setDialog('stop')}>■ Stop</button>
        <button className="button danger" onClick={() => setDialog('emergency')}>🛑 Emergency stop</button>
      </div>
      {dialog === 'stop' && <ConfirmDialog title="Stop the bot?" body={status.managed ? 'The managed bot process will receive SIGTERM.' : 'An external bot process is running. Allowing external stop will send it SIGTERM.'} confirmLabel="Stop bot" danger onClose={() => setDialog(null)} onConfirm={() => { setDialog(null); void action('/api/bot/stop', { allow_external: !status.managed }, 'Bot stop requested') }} />}
      {dialog === 'emergency' && <ConfirmDialog title="Emergency stop" body="This disables trading and stops the bot immediately." confirmLabel="Emergency stop" danger requireText="EMERGENCY STOP" onClose={() => setDialog(null)} onConfirm={() => { setDialog(null); void action('/api/bot/emergency-stop', { confirm_phrase: 'EMERGENCY STOP', close_positions: closePositions }, 'Emergency stop requested') }}><label className="checkbox-row"><input type="checkbox" checked={closePositions} onChange={(event) => setClosePositions(event.target.checked)} /> Also close all open positions on Bybit</label></ConfirmDialog>}
      {dialog === 'reset' && <ConfirmDialog title="Reset emergency stop?" body="Trading will be enabled and the paused state cleared." confirmLabel="Reset emergency" onClose={() => setDialog(null)} onConfirm={() => { setDialog(null); void action('/api/bot/reset-emergency', { confirm: true }, 'Emergency stop reset') }} />}
      {dialog === 'kill' && <ConfirmDialog title="Reset kill switch?" body="The persisted kill-switch state will be cleared." confirmLabel="Reset kill switch" onClose={() => setDialog(null)} onConfirm={() => { setDialog(null); void action('/api/bot/kill-switch/reset', { confirm: true }, 'Kill switch reset') }} />}
      <div className="alerts">
        {status.state === 'KILL_SWITCH' && <div className="alert warning"><strong>Kill switch triggered</strong><span>{status.control.kill_switch_reason ?? status.kill_switch.triggered_reason ?? 'Daily loss limit reached'}</span><button className="button tiny" onClick={() => setDialog('kill')}>Reset kill switch</button></div>}
        {status.state === 'EMERGENCY_STOP' && <div className="alert danger"><strong>Emergency stop active</strong><span>Trading is disabled until manually reset.</span><button className="button tiny danger" onClick={() => setDialog('reset')}>Reset emergency</button></div>}
      </div>
    </>
  )
}
  ...
  return (
    <>
      <PageHeader eyebrow="OPERATIONS" title="Dashboard">
        <div className="header-summary"><StatusBadge status={status} /><span className={clsx('chip', status.bybit.connected ? 'success' : 'danger')}>{status.bybit.connected ? '● Bybit OK' : '● CONNECTION ERROR'}</span><span className="chip">Heartbeat {fmtAge(status.heartbeat_age_sec)}</span><span className="chip">Uptime {fmtDuration(status.uptime_sec)}</span>{status.strategy_id && <span className="chip">{status.strategy_id}</span>}</div>
      </PageHeader>
      ...
      <div className="kpi-grid">
        <KpiCard label="Balance" value={fmtUsd(summary.wallet.wallet_balance)} />
        <KpiCard label="Equity" value={fmtUsd(equity)} tone={toneFor(equity) === 'profit' ? 'profit' : 'neutral'} />
        <KpiCard label="Available" value={fmtUsd(summary.wallet.available_balance)} />
        <KpiCard label="Unrealized PnL" value={fmtUsd(unrealized)} tone={toneFor(unrealized) === 'profit' ? 'profit' : toneFor(unrealized) === 'loss' ? 'loss' : 'neutral'} />
        <KpiCard label="Realized PnL" value={fmtUsd(summary.realized_pnl_all)} tone={toneFor(summary.realized_pnl_all) === 'profit' ? 'profit' : 'loss'} />
        <KpiCard label="PnL Today" value={fmtUsd(light?.pnl_today ?? summary.pnl_today)} tone={toneFor(light?.pnl_today ?? summary.pnl_today) === 'profit' ? 'profit' : 'loss'} />
        ...
        <KpiCard label="Drawdown" value={fmtUsd(summary.drawdown.usd)} sub={summary.drawdown_source ? `${fmtPct(summary.drawdown.pct)} from 90d equity peak` : 'no equity history yet'} tone="loss" />
        <KpiCard label="Open positions" value={light?.open_positions ?? summary.open_positions} />
        <KpiCard label="Trades today" value={summary.trades_today} />
        <KpiCard label="Daily loss" value={`${fmtUsd(-dailyLoss)} (${fmtPct(-dailyLossPct)})`} sub={`limit ${fmtPct(summary.risk.max_daily_loss_pct)}`} tone={dailyLoss < 0 ? 'warning' : 'neutral'} />
      </div>
      <div className="two-column">
        <Panel title="Recent events"><EventList events={events.slice(0, 15)} /></Panel>
        <Panel title="Connection details"><div className="detail-list"><div><span>Process</span><strong>{status.process_running ? `PID ${status.pid ?? '—'}` : 'Stopped'}</strong></div><div><span>Heartbeat</span><strong>{fmtAge(status.heartbeat_age_sec)}</strong></div><div><span>Cycle</span><strong>{status.cycle_ms ? `${status.cycle_ms.toFixed(0)} ms` : '—'}</strong></div><div><span>Last exit</span><strong>{status.last_exit_code ?? '—'}</strong></div><div><span>Trading enabled</span><strong>{status.control.trading_enabled ? 'Yes' : 'No'}</strong></div></div></Panel>
      </div>
    </>
  )
}
...
export function PositionsPage() {
  ...
  return (
    <>
      <PageHeader eyebrow="RISK EXPOSURE" title="Positions"><span className="chip">{positions.length} open</span></PageHeader>
...
export function StrategySettingsPage() {
  ...
  return <><PageHeader eyebrow="STRATEGY SETTINGS" title={strategyId}><Link className="button subtle" to="/strategies">← Back</Link></PageHeader><div className="alert info">Settings apply on bot restart ({query.data?.applies_on ?? 'restart'}).</div>{query.data && <SettingsForm data={query.data} />}{query.isPending && <div className="panel-loading">Loading settings…</div>}{query.error && <ErrorState message={query.error.message} />}</>
}
...
function ChartPanel({ title, data, dataKey, color, bar = false, area = false, empty }: { title: string; data: Array<Record<string, unknown>>; dataKey: string; color: string; bar?: boolean; area?: boolean; empty?: string }) {
  return <Panel title={title}>{data.length ? <div className="chart"><ResponsiveContainer width="100%" height={260}>{bar ? <BarChart data={data}><CartesianGrid stroke="#1e293b" vertical={false} /><XAxis dataKey="date" stroke="#64748b" /><YAxis stroke="#64748b" /><Tooltip contentStyle={{ background: '#0f172a', border: '1px solid #334155' }} /><Bar dataKey={dataKey} fill={color} radius={[4, 4, 0, 0]} /></BarChart> : area ? <AreaChart data={data}><CartesianGrid stroke="#1e293b" vertical={false} /><XAxis dataKey="ts" stroke="#64748b" /><YAxis stroke="#64748b" /><Tooltip contentStyle={{ background: '#0f172a', border: '1px solid #334155' }} /><Area dataKey={dataKey} stroke={color} fill={`${color}33`} /></AreaChart> : <LineChart data={data}><CartesianGrid stroke="#1e293b" vertical={false} /><XAxis dataKey="ts" stroke="#64748b" /><YAxis stroke="#64748b" /><Tooltip contentStyle={{ background: '#0f172a', border: '1px solid #334155' }} /><Line type="monotone" dataKey={dataKey} stroke={color} dot={false} strokeWidth={2} /></LineChart>}</ResponsiveContainer></div> : <Empty text={empty ?? 'No chart data'} />}</Panel>
}
...
export function TradesPage() {
  ...
  const queryString = new URLSearchParams({ period, limit: '50', offset: String(page * 50), ...(symbol ? { symbol } : {}), ...(strategy ? { strategy } : {}), ...(side ? { side } : {}), ...(result ? { result } : {}), ...(exitReason ? { exit_reason: exitReason } : {}) }).toString()
  const query = useQuery({ queryKey: ['trades', queryString], queryFn: () => getTrades(queryString) })
  const [selected, setSelected] = useState<Trade | null>(null)
  const filters = query.data?.filters
  return <><PageHeader eyebrow="JOURNAL" title="Trades"><PeriodPicker value={period} onChange={(value) => { setPeriod(value); setPage(0) }} /></PageHeader><div className="filter-bar"><select value={symbol} onChange={(event) => setSymbol(event.target.value)}><option value="">All symbols</option>{filters?.symbols.map((item) => <option key={item}>{item}</option>)}</select><select value={strategy} onChange={(event) => setStrategy(event.target.value)}><option value="">All strategies</option>{filters?.strategies.map((item) => <option key={item}>{item}</option>)}</select><select value={side} onChange={(event) => setSide(event.target.value)}><option value="">All sides</option><option value="long">Long</option><option value="short">Short</option></select><select value={result} onChange={(event) => setResult(event.target.value)}><option value="">All results</option><option value="win">Win</option><option value="loss">Loss</option></select><select value={exitReason} onChange={(event) => setExitReason(event.target.value)}><option value="">All exit reasons</option>{filters?.exit_reasons.map((item) => <option key={item}>{item}</option>)}</select></div><Panel title={`${query.data?.total ?? 0} trades`}><LoadingOrError loading={query.isPending} error={query.error} /><DataTable rows={query.data?.items ?? []} rowKey={(row) => row.trade_id} onRowClick={setSelected} renderCard={(row) => <><div className="card-line"><strong>{row.symbol}</strong><span>{row.strategy_id}</span><time>{fmtTs(row.closed_ts)}</time></div><div className="card-grid"><span>Side <b>{row.side}</b></span><span>PnL <b className={toneFor(row.pnl)}>{fmtUsd(row.pnl)}</b></span><span>R <b>{numberValue(row.r_multiple).toFixed(2)}</b></span><span>Duration <b>{fmtDuration(row.duration_sec)}</b></span></div></>} columns={[{ key: 'date', label: 'Date', render: (row) => fmtTs(row.closed_ts) }, { key: 'symbol', label: 'Symbol', render: (row) => <strong>{row.symbol}</strong> }, { key: 'strategy', label: 'Strategy', render: (row) => row.strategy_id ?? '—' }, { key: 'side', label: 'Side', render: (row) => row.side }, { key: 'entry', label: 'Entry', render: (row) => fmtPrice(row.entry_price) }, { key: 'exit', label: 'Exit', render: (row) => fmtPrice(row.exit_price) }, { key: 'qty', label: 'Size', render: (row) => row.qty ?? '—' }, { key: 'fees', label: 'Fees', render: (row) => `${fmtUsd(row.fees_actual ?? row.fees_est)}${row.fees_actual == null ? ' est' : ''}` }, { key: 'pnl', label: 'PnL', render: (row) => <span className={toneFor(row.pnl)}>{fmtUsd(row.pnl)}</span> }, { key: 'r', label: 'R', render: (row) => numberValue(row.r_multiple).toFixed(2) }, { key: 'reason', label: 'Exit reason', render: (row) => row.exit_reason ?? '—' }]} /></Panel><div className="pagination"><button className="button tiny subtle" disabled={page === 0} onClick={() => setPage(page - 1)}>← Previous</button><span>Page {page + 1}</span><button className="button tiny subtle" disabled={!query.data || (page + 1) * 50 >= query.data.total} onClick={() => setPage(page + 1)}>Next →</button></div>{selected && <TradeDrawer trade={selected} onClose={() => setSelected(null)} />}</>
}
