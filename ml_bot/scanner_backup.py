# Вставь код, сохрани (Ctrl+X, Y, Enter)
./run_bot.sh"""
...
LEVERAGE = 1
TP_PERCENT = 0.002         # 0.2% take profit (simple limit order)
SL_PERCENT = 0.002         # 0.2% stop loss (simple limit order)
PROB_THRESHOLD = 0.55
UNCERTAINTY_THRESHOLD = 0.05  # Skip if prob difference < 5%
MIN_TREND_STRENGTH = 0    # Minimum trend strength to trade
...
def _check_tpsl(ex, tracker, logger, predictions=None):
    """Check early exit for all open positions. TP/SL handled by exchange limit orders.
                        ...
                        if signal:
                            ...
                            if not has_position:
                                ...
                                try:
                                    ...
                                    # Set TP and SL orders on exchange immediately (simple 0.2% limit orders)
                                    try:
                                        if signal == 'long':
                                            ...
                                            side_str = 'Sell'
                                        # Set TP order
                                        ex.limit_order(sym, side_str, qty, tp_price, reduce_only=False)
                                        logger.info(f"  >> TP ORDER SET for {sym} @ {tp_price:.4f} (0.2% take profit)")
                                        # Set SL order (limit order)
                                        sl_side = 'Sell' if signal == 'long' else 'Buy'
                                        ex.limit_order(sym, sl_side, qty, sl_price, reduce_only=True)
                                        logger.info(f"  >> SL ORDER SET for {sym} @ {sl_price:.4f} (0.2% stop loss)")
                                    except Exception as e:
                                        logger.error(f"  >> FAILED to set TP/SL for {sym}: {e}")
                                    
                                    tracker.reset(sym)
                                    # Small delay to ensure TP/SL orders are registered before continuing
                                    time.sleep(0.05)
                                except Exception as e:
                                    err_str = str(e)
                                        ...
                                        if match:
                                            ...
                                            if retry_qty_final:
                                                ...
                                                try:
                                                    ...
                                                    # Set TP and SL orders on exchange immediately
                                                    try:
                                                        if signal == 'long':
                                                            tp_price = current_price * (1 + TP_PERCENT)
                                                            sl_price = current_price * (1 - SL_PERCENT)
                                                        ...
                                                        sl_side = 'Sell' if signal == 'long' else 'Buy'
                                                        ex.limit_order(sym, sl_side, qty, sl_price, reduce_only=True)
                                                        logger.info(f"  >> SL ORDER SET for {sym} @ {sl_price:.4f} (retry)")
                                                    except Exception as e:
                                                        logger.error(f"  >> FAILED to set TP/SL for {sym}: {e}")
