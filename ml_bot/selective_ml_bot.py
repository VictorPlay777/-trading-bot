...
class PositionState:
    ...
    # v8: Target PnL for percentage-based TP/SL (1% of position notional)
    target_pnl: float = 0.0
    # v8: Trailing TP/SL watermarks for symmetric trailing
    high_watermark_pnl: float = 0.0  # Maximum unreal PnL seen (for profit trailing)
    low_watermark_pnl: float = 0.0   # Minimum unreal PnL seen (for loss trailing)
    trailing_active: bool = False     # Whether trailing is active
    # v9.1: Marker for positions adopted from exchange without bot's own signal metadata.
    # Such positions get force-closed if they hit a profit threshold to avoid being stuck.
...
class SelectiveMLBot:
    def __init__(self, cfg_path: str):
        ...
        # v8: load symbol statistics for winrate-based blacklist
        self._stats_by_symbol = {}
        self._blacklist_path = Path("ml_bot/logs/symbol_blacklist.json")
        self._blacklist_path.parent.mkdir(parents=True, exist_ok=True)
        self._blacklist = set()  # Runtime blacklist from file
        self._load_symbol_stats()
        self._load_blacklist_from_file()  # Load persistent blacklist from file
        self._preload_blacklist()
        ...
        # v10: paths can be overridden via config attributes (used by V10 subclass).
        # Defaults preserve v9.1 behaviour exactly.
        self._state_path = Path(getattr(self.prod, "v10_state_path", None) or "logs/last_state.json")
        self._health_path = Path(getattr(self.prod, "v10_health_path", None) or "logs/health.json")
        self._trades_log_path = Path(
            getattr(self.prod, "v10_trades_path", None)
            or (Path(__file__).parent / "logs" / "trades.jsonl")
        )
        ...
        # v8: periodically reload symbol stats
        self._last_stats_update = 0.0
        self._stats_update_interval_sec = 300.0  # 5 minutes
        # v9.1: Restore position state from disk so TP/SL/signal_meta survive restarts.
        self._load_state()

    def _load_symbol_stats(self):
        """Load symbol statistics from stats_by_symbol.json for winrate filtering."""
        try:
            stats_path = Path("ml_bot/logs/stats_by_symbol.json")
            if stats_path.exists():
                self._stats_by_symbol = json.loads(stats_path.read_text(encoding="utf-8"))
                logger.info(f"[V8] Loaded stats for {len(self._stats_by_symbol)} symbols")
            else:
                self._stats_by_symbol = {}
                logger.info("[V8] No stats_by_symbol.json found, starting empty")
        except Exception as e:
            logger.warning(f"[V8] Failed to load symbol stats: {e}")
            self._stats_by_symbol = {}

    def _update_symbol_stats(self):
        """Periodically reload symbol statistics to catch new trades."""
        self._load_symbol_stats()

    def _load_blacklist_from_file(self):
        """Load blacklist from file on startup. v9.1: tolerate broken JSON."""
        if getattr(self.prod, "disable_blacklist", False):
            self._blacklist = set()
            logger.info("[V9.1] blacklist disabled by config - skip load")
            return
        try:
            if self._blacklist_path.exists():
                txt = self._blacklist_path.read_text(encoding="utf-8").strip()
                # Defensive: file may contain concatenated JSON objects from a buggy writer.
                # Try to parse as-is, fall back to extracting first valid JSON object.
                try:
                    blacklist = json.loads(txt)
                except json.JSONDecodeError as je:
                    logger.warning(f"[V9.1] blacklist file is broken ({je}); resetting to empty")
                    blacklist = {}
                    try:
                        self._blacklist_path.write_text("{}", encoding="utf-8")
                    except Exception:
                        pass
                self._blacklist = set(blacklist.keys()) if isinstance(blacklist, dict) else set()
                logger.info(f"[V9.1] Loaded {len(self._blacklist)} symbols from blacklist file")
            else:
                logger.info("[V9.1] No blacklist file found, starting with empty blacklist")
        except Exception as e:
            logger.error(f"[V9.1] Failed to load blacklist from file: {e}")
            self._blacklist = set()

    def _add_to_blacklist(self, symbol: str, reason: str):
        """Add symbol to blacklist file. v9.1: no-op when disabled."""
        if getattr(self.prod, "disable_blacklist", False):
            return
        try:
            blacklist = {}
            if self._blacklist_path.exists():
                try:
                    blacklist = json.loads(self._blacklist_path.read_text(encoding="utf-8"))
            ...
            blacklist[symbol] = {
                "reason": reason,
                "timestamp": time.time(),
                "total_trades": self._stats_by_symbol.get(symbol, {}).get("total_trades", 0),
                "winrate": self._stats_by_symbol.get(symbol, {}).get("winrate", 0.0),
                "avg_pnl": self._stats_by_symbol.get(symbol, {}).get("avg_pnl", 0.0),
            }
            self._blacklist_path.write_text(json.dumps(blacklist, indent=2, ensure_ascii=False), encoding="utf-8")
            self._blacklist.add(symbol)
            logger.warning(f"[V9.1] Added {symbol} to blacklist: {reason}")
        except Exception as e:
            logger.error(f"[V9.1] Failed to add to blacklist: {e}")

    def _preload_blacklist(self):
        """Preload blacklist from stats. v9.1: skipped when disabled."""
        if getattr(self.prod, "disable_blacklist", False):
            logger.info("[V9.1] blacklist disabled by config - skip preload")
            return
        try:
            added_count = 0
            for symbol, stats in self._stats_by_symbol.items():
                total_trades = stats.get("total_trades", 0)
                winrate = stats.get("winrate", 0.0)
                avg_pnl = stats.get("avg_pnl", 0.0)
                if total_trades >= 10 and winrate < 0.50 and avg_pnl < 0:
                    reason = f"low_winrate_negative_pnl(trades={total_trades}, winrate={winrate*100:.1f}%, avg_pnl={avg_pnl:.2f})"
                    self._add_to_blacklist(symbol, reason)
                    added_count += 1
            logger.info(f"[V9.1] Preloaded blacklist: added {added_count} symbols")
        except Exception as e:
            logger.error(f"[V9.1] Failed to preload blacklist: {e}")
    ...
    def _rebuild_brackets_for_state(self, st: PositionState):
        ...
        # Calculate current position notional for percentage-based TP/SL
        position_notional = float(st.qty_total * Decimal(str(st.entry_price)))
        sig = {"symbol": st.symbol, "direction": st.side, "entry": st.entry_price, "atr": atr_v}
        levels = self._build_exit_levels(sig, position_notional=position_notional)
        ...
        # v8: Recalculate target PnL for percentage-based TP/SL
        percentage_based = getattr(self.prod, "tp_sl_percentage_based", False)
        tp_sl_percentage = getattr(self.prod, "tp_sl_percentage", 0.01)
        if percentage_based:
            st.target_pnl = position_notional * tp_sl_percentage
    ...
    def select_symbols(self):
        ...
        for t in tickers:
            ...
            if not sym.endswith("USDT"):
                continue
            # v8: Skip blacklisted symbols during scanning
    ...
    def train_and_predict(self, symbol: str, pre_ob: dict = None):
        ...
        # v10b: always compute ADX for research even if filter is off (min_adx=0).
        adx_val: float = 0.0
        if getattr(self.prod, "always_compute_research_metrics", False):
            try:
                adx_val = float(self._calculate_adx(symbol) or 0.0)
            except Exception:
                adx_val = 0.0
        return {
            "symbol": symbol,
    ...
    def _build_exit_levels(self, sig, position_notional: float = 0.0):
        # v8: Percentage-based TP/SL or ATR-based TP/SL
        percentage_based = getattr(self.prod, "tp_sl_percentage_based", False)
        tp_sl_percentage = getattr(self.prod, "tp_sl_percentage", 0.01)

        if percentage_based and position_notional > 0:
            # Percentage-based mode: use fixed percentage of position notional
            brackets = self.exit_engine.compute_brackets(
                sig["direction"], sig["entry"], sig["atr"], self.prod.sl_atr_mult,
                self.prod.tp1_r, self.prod.tp2_r,
                position_notional=position_notional,
                percentage_based=percentage_based,
                tp_sl_percentage=tp_sl_percentage
            )
            # TP3 equals TP1 in percentage-based mode
            brackets["tp3"] = brackets["tp1"]
            logger.info(
                f"[PERCENTAGE_TP/SL] {sig['symbol']} position_notional=${position_notional:.0f} "
                f"percentage={tp_sl_percentage*100:.1f}% target_pnl=${position_notional * tp_sl_percentage:.0f} "
                f"tp1={brackets['tp1']:.6f} sl={brackets['sl']:.6f}"
            )
        else:
            # ATR-based mode with dynamic TP scaling
    ...
    def _create_state(self, sig, qty: Decimal, ex_row: dict = None):
        ...
        # Calculate position notional for percentage-based TP/SL
        position_notional = float(qty * Decimal(str(sig["entry"])))
        levels = self._build_exit_levels(sig, position_notional=position_notional)
        st = PositionState(
            symbol=sig["symbol"],
            side=sig["direction"],
        ...
        # v8: Save target PnL for percentage-based TP/SL
        percentage_based = getattr(self.prod, "tp_sl_percentage_based", False)
        tp_sl_percentage = getattr(self.prod, "tp_sl_percentage", 0.01)
        if percentage_based:
            st.target_pnl = position_notional * tp_sl_percentage
        ...
        # v10 hook: subclass can snapshot features and start MFE/MAE tracking here.
        try:
            self._v10_hook_position_opened(st, sig)
        except Exception as _e:
            logger.debug(f"[V10 HOOK] position_opened failed: {_e}")

    # ------------------------------------------------------------------
    # V10 research hooks. No-ops in v9.1; overridden by V10MLBot subclass.
    # ------------------------------------------------------------------
    def _v10_hook_signal_evaluated(self, sig: dict, allow: bool, reason: str, size_mult: float) -> None:
        """Called for every signal evaluation (allowed and rejected). No-op in v9.1."""
        return None

    def _v10_hook_position_opened(self, st: "PositionState", sig: dict) -> None:
        """Called once per real position open. No-op in v9.1."""
        return None

    def _v10_hook_price_tick(self, symbol: str, price: float, st) -> None:
        """Called every monitor cycle with the freshest mark price. No-op in v9.1."""
        return None

    def _v10_hook_trade_closed_record(self, st: "PositionState", record: dict) -> None:
        """Called just before writing the trade record. No-op in v9.1."""
        return None
    ...
    def _close_market_reduce_only(self, st: PositionState, qty: Decimal, reason: str):
        ...
        if int(res.get("retCode", -1)) != 0:
            ...
            return False
        # v9.1: Bybit may partially fill market reduce (max order qty per request),
        # so the requested qty is an upper bound on what was actually closed.
        # Cap qty_closed at qty_total to prevent the negative-remaining drift that
        # leaves positions stuck (qty_closed > qty_total -> _remaining_qty=0 -> monitor skips).
        # The next _sync_state_from_exchange cycle will reconcile qty_total to the real
        # exchange size, so any unclosed remainder gets re-attempted on subsequent cycles.
        new_closed = st.qty_closed + Decimal(str(qty))
        if new_closed > st.qty_total:
            logger.warning(
                f"[EXIT CAP] {st.symbol} requested_close={qty} would push qty_closed={new_closed} "
                f"above qty_total={st.qty_total}; capping at qty_total (partial-fill expected)"
            )
            new_closed = st.qty_total
        st.qty_closed = new_closed
    ...
    def _sync_state_from_exchange(self, positions):
        ...
        for p in positions:
            ...
            if sym not in self.position_states:
                # v9.1: Compute realistic ATR from OHLCV instead of fake entry*0.004
                # so the TP/SL brackets for adopted positions reflect real volatility.
                try:
                    df = self._cached_ohlcv(sym, limit=600)
                    atr_v = atr(df, 14)
                    if not atr_v or atr_v <= 0:
                        atr_v = entry * 0.004
                except Exception:
                    atr_v = entry * 0.004
                sig = {"symbol": sym, "direction": side, "entry": entry, "atr": atr_v}
                self._create_state(sig, sz, ex_row=p)
                st_new = self.position_states[sym]
                st_new.exit_state = "open"
                # v9.1: Mark as orphan - no original signal context, may need force-close on profit.
                st_new.is_orphan = True
                # v9.1: Prefer exchange-side TP/SL if Bybit reports them (string fields).
                try:
                    ex_tp = float(p.get("takeProfit") or 0)
                    ex_sl = float(p.get("stopLoss") or 0)
                    if ex_tp > 0:
                        st_new.take_profit_levels = {"tp1": ex_tp, "tp2": ex_tp, "tp3": ex_tp}
                        st_new.trailing_price = ex_tp
                    if ex_sl > 0:
                        st_new.stop_loss_price = ex_sl
                except Exception:
                    pass
                logger.warning(
                    f"[ORPHAN_ADOPTED] {sym} side={side} entry={entry} qty={sz} "
                    f"atr={atr_v:.6g} sl={st_new.stop_loss_price:.6g} tp1={st_new.take_profit_levels.get('tp1'):.6g} "
                    f"ex_tp={p.get('takeProfit')} ex_sl={p.get('stopLoss')}"
                )
            else:
                st = self.position_states[sym]
                prev_sz = st.last_exchange_size
                # v9.1: Heal qty_closed/qty_total drift. If exchange has a live position but
                # our local accounting says nothing remains (qty_closed >= qty_total) or the
                # state is marked closed, reset the exit accounting so the monitor processes
                # this position again. Otherwise the orphan force-close (and TP/SL logic)
                # never runs because _remaining_qty() returns 0 and _monitor_positions
                # short-circuits with `continue`.
                try:
                    rem_local = max(Decimal("0"), st.qty_total - st.qty_closed)
                except Exception:
                    rem_local = Decimal("0")
                if sz > 0 and (rem_local <= 0 or st.exit_state == "closed" or st.qty_closed > st.qty_total):
                    logger.warning(
                        f"[STATE HEAL] {sym} exchange_size={sz} but local qty_total={st.qty_total} "
                        f"qty_closed={st.qty_closed} exit_state={st.exit_state} - reset accounting"
                    )
                    st.qty_total = sz
                    ...
                    st.exit_state = "open"
                    st.tp1_done = st.tp2_done = st.tp3_done = False
                    # Mark as orphan so the force-close-on-profit logic kicks in.
                    st.is_orphan = True
                ...
                if side_changed or size_changed or avg_changed:
                    ...
                    st.side = side
                    # v8: Always update qty_total for percentage-based TP/SL to use actual exchange position size
                    st.qty_total = sz
                    if side_changed:
    ...
    def _record_closed_trade(self, st: PositionState):
        ...
        # v10 hook: allow subclass to enrich the record before write (e.g. add MFE/MAE/feature_hash).
        try:
            self._v10_hook_trade_closed_record(st, record)
        except Exception as _e:
            logger.debug(f"[V10 HOOK] trade_closed_record failed: {_e}")
        try:
            # v10: path is config-driven (self._trades_log_path). Fallback in __init__.
            trade_log_path = self._trades_log_path
    ...
    def _save_state(self):
        """Persist full position state to disk so it survives restarts (v9.1)."""
        payload = {
            "timestamp": time.time(),
            "positions": {
                sym: {
                    ...
                    "qty_total": str(st.qty_total),
                    "qty_closed": str(st.qty_closed),
                    "qty_original": str(st.qty_original),
                    "entry_fees": st.entry_fees,
                    "exit_fees_estimate": st.exit_fees_estimate,
                    "funding_estimate": st.funding_estimate,
                    ...
                    "last_exchange_size": str(st.last_exchange_size),
                    "cum_realised_pnl": st.cum_realised_pnl,
                    "cum_realised_pnl_entry": st.cum_realised_pnl_entry,
                    "position_value_exchange": st.position_value_exchange,
                    "opened_ts": st.opened_ts,
                    "signal_meta": st.signal_meta,
                    "exit_reasons": st.exit_reasons,
                    "target_pnl": st.target_pnl,
                    "high_watermark_pnl": st.high_watermark_pnl,
                    "low_watermark_pnl": st.low_watermark_pnl,
                    "trailing_active": st.trailing_active,
                    "is_orphan": st.is_orphan,
                }
    ...
    def _load_state(self):
        """Load persisted position state from disk on startup (v9.1).
        Restores PositionState for positions that still exist on the exchange.
        """
        try:
            if not self._state_path.exists():
                logger.info("[STATE LOAD] no last_state.json - starting fresh")
                return
            payload = json.loads(self._state_path.read_text(encoding="utf-8"))
            saved_ts = float(payload.get("timestamp", 0) or 0)
            age = time.time() - saved_ts
            positions_data = payload.get("positions", {}) or {}
            if not positions_data:
                logger.info(f"[STATE LOAD] empty positions in last_state.json (age={age:.0f}s)")
                return
            # Stale guard: if state is older than 24h, skip to avoid rehydrating obsolete data
            if age > 86400:
                logger.warning(f"[STATE LOAD] last_state.json too old ({age:.0f}s) - skip restore")
                return
            restored = 0
            for sym, d in positions_data.items():
                try:
                    st = PositionState(
                        ...
                        qty_total=Decimal(str(d.get("qty_total", "0"))),
                        qty_closed=Decimal(str(d.get("qty_closed", "0"))),
                        entry_fees=float(d.get("entry_fees", 0.0) or 0.0),
                        exit_fees_estimate=float(d.get("exit_fees_estimate", 0.0) or 0.0),
                        funding_estimate=float(d.get("funding_estimate", 0.0) or 0.0),
                        stop_loss_price=float(d.get("stop_loss_price", 0.0) or 0.0),
                        take_profit_levels=dict(d.get("take_profit_levels") or {}),
                        exit_state=d.get("exit_state", "open"),
                        tp1_done=bool(d.get("tp1_done", False)),
                        tp2_done=bool(d.get("tp2_done", False)),
                        tp3_done=bool(d.get("tp3_done", False)),
                        trailing_price=float(d.get("trailing_price", 0.0) or 0.0),
                        updated_ts=float(d.get("updated_ts", 0.0) or 0.0),
                        leverage=int(d.get("leverage", 0) or 0),
                        last_exchange_size=Decimal(str(d.get("last_exchange_size", "0"))),
                        cum_realised_pnl=float(d.get("cum_realised_pnl", 0.0) or 0.0),
                        position_value_exchange=float(d.get("position_value_exchange", 0.0) or 0.0),
                        opened_ts=float(d.get("opened_ts", 0.0) or 0.0),
                        signal_meta=dict(d.get("signal_meta") or {}),
                        exit_reasons=list(d.get("exit_reasons") or []),
                        cum_realised_pnl_entry=float(d.get("cum_realised_pnl_entry", 0.0) or 0.0),
                        qty_original=Decimal(str(d.get("qty_original", d.get("qty_total", "0")))),
                        target_pnl=float(d.get("target_pnl", 0.0) or 0.0),
                        high_watermark_pnl=float(d.get("high_watermark_pnl", 0.0) or 0.0),
                        low_watermark_pnl=float(d.get("low_watermark_pnl", 0.0) or 0.0),
                        trailing_active=bool(d.get("trailing_active", False)),
                        is_orphan=bool(d.get("is_orphan", False)),
                    )
                    # v9.1: If signal_meta is empty, treat as orphan even if flag was missing/false.
                    # This catches positions saved by older code versions or adopted from exchange.
                    if not st.signal_meta or not any(st.signal_meta.values()):
                        st.is_orphan = True
                    self.position_states[sym] = st
                    restored += 1
                except Exception as e:
                    logger.warning(f"[STATE LOAD] failed to restore {sym}: {e}")
            logger.info(f"[STATE LOAD] restored {restored} positions from last_state.json (age={age:.0f}s)")
        except Exception as e:
    ...
    def _monitor_positions(self):
        ...
        for sym, st in list(self.position_states.items()):
            try:
                ...
                if price <= 0:
                    continue
                # v10 hook: feed mark price to MFE/MAE tracker + shadow tracker.
                try:
                    self._v10_hook_price_tick(sym, price, st)
                except Exception as _e:
                    logger.debug(f"[V10 HOOK] price_tick failed: {_e}")
                ...
                # v9.1: Force-close orphan positions on profit threshold.
                # Adopted (orphan) positions don't have reliable signal/ATR context, so we
                # don't trust their internal TP brackets. Once they reach a meaningful
                # profit (configurable, default +1% of notional), close them market.
                if st.is_orphan:
                    orphan_thr = float(getattr(self.prod, "orphan_profit_close_pct", 0.01) or 0.01)
                    notional_entry = float(st.entry_price) * float(st.qty_total or 0)
                    profit_target = notional_entry * orphan_thr
                    pnl_for_check = unreal if unreal_ex is not None else unreal_math
                    if notional_entry > 0 and pnl_for_check >= profit_target:
                        logger.warning(
                            f"[ORPHAN_FORCE_CLOSE] {sym} unreal_pnl={pnl_for_check:.2f} "
                            f">= target={profit_target:.2f} ({orphan_thr*100:.2f}% of notional={notional_entry:.2f})"
                        )
                        if self._close_market_reduce_only(st, rem, "orphan_force_close_profit"):
                            st.exit_state = "closed"
                        continue
                ...
                # v8: Use PnL-based TP/SL for percentage-based mode
                percentage_based = getattr(self.prod, "tp_sl_percentage_based", False)
                if percentage_based and st.target_pnl > 0:
                    # Check unreal PnL from exchange instead of price
                    unreal_pnl = unreal_ex if unreal_ex is not None else unreal_math
                    trailing_enabled = getattr(self.prod, "trailing_enabled", False)
                    trailing_start = getattr(self.prod, "trailing_start_pnl_pct", 0.01) * st.target_pnl
                    trailing_distance = getattr(self.prod, "trailing_distance_pct", 0.005) * st.target_pnl

                    # v8: Symmetric trailing TP/SL logic
                    if trailing_enabled:
                        # Update watermarks
                        if unreal_pnl > st.high_watermark_pnl:
                            st.high_watermark_pnl = unreal_pnl
                        if unreal_pnl < st.low_watermark_pnl or st.low_watermark_pnl == 0.0:
                            st.low_watermark_pnl = unreal_pnl

                        # Check if trailing should start
                        if not st.trailing_active:
                            if abs(unreal_pnl) >= trailing_start:
                                st.trailing_active = True
                                # Initialize watermarks
                                st.high_watermark_pnl = max(unreal_pnl, 0.0)
                                st.low_watermark_pnl = min(unreal_pnl, 0.0)

                        # Trailing logic
                        if st.trailing_active:
                            # PROFIT trailing: close if unreal_pnl falls below (high_watermark - distance)
                            if unreal_pnl > 0:
                                trailing_sl = st.high_watermark_pnl - trailing_distance
                                tp_hit = unreal_pnl <= trailing_sl
                                sl_pnl_hit = False  # No hard SL in profit mode
                            # LOSS trailing: close if unreal_pnl falls below (low_watermark - distance)
                            else:
                                trailing_sl = st.low_watermark_pnl - trailing_distance
                                tp_hit = False  # No TP in loss mode
                                sl_pnl_hit = unreal_pnl <= trailing_sl

                            logger.info(
                                f"[TRAILING_TP/SL] {sym} unreal_pnl={unreal_pnl:.2f} "
                                f"high_watermark={st.high_watermark_pnl:.2f} low_watermark={st.low_watermark_pnl:.2f} "
                                f"trailing_sl={trailing_sl:.2f} tp_hit={tp_hit} sl_pnl_hit={sl_pnl_hit}"
                            )
                        else:
                            # Not yet trailing - use simple TP/SL
                            tp_hit = unreal_pnl >= st.target_pnl
                            sl_pnl_hit = unreal_pnl <= -st.target_pnl
                            logger.info(
                                f"[PNL_TP/SL] {sym} unreal_pnl={unreal_pnl:.2f} target_pnl={st.target_pnl:.2f} "
                                f"tp_hit={tp_hit} sl_pnl_hit={sl_pnl_hit} trailing_active={st.trailing_active}"
                            )
                    else:
                        # Original simple PnL-based TP/SL (no trailing)
                        tp_hit = unreal_pnl >= st.target_pnl
                        sl_pnl_hit = unreal_pnl <= -st.target_pnl
                        logger.info(
                            f"[PNL_TP/SL] {sym} unreal_pnl={unreal_pnl:.2f} target_pnl={st.target_pnl:.2f} "
                            f"tp_hit={tp_hit} sl_pnl_hit={sl_pnl_hit} tp1_done={st.tp1_done}"
                        )

                    if sl_pnl_hit and not st.tp1_done:
                        q = self._fit_exit_qty(st, self._remaining_qty(st))
                        if self._close_market_reduce_only(st, q, "stop_loss_pnl"):
                            st.tp1_done = True
                            st.tp2_done = True
                            st.tp3_done = True
                            st.exit_state = "closed"
                    ...
                    if single_tp_full:
                        if tp_hit and not st.tp1_done:
                            q = self._fit_exit_qty(st, self._remaining_qty(st))
                            if self._close_market_reduce_only(st, q, "tp1_full_pnl"):
                                st.tp1_done = True
                                st.tp2_done = True
                                st.tp3_done = True
                                st.exit_state = "closed"
                    ...
                    # Original price-based TP/SL logic
    ...
    def allowed(self, sig, open_positions, equity, used_notional):
        # V8: ABSOLUTE CHECK - Block adding to positions with negative unreal PnL (nothing can override this)
        symbol = sig["symbol"]
        # V10c: Block adding to existing positions if allow_add_to_position=False (research mode: 1 signal = 1 trade)
        # Check Bybit API directly to prevent race condition (local state may be stale during rapid signals)
        if not getattr(self.prod, "allow_add_to_position", True):
            try:
                pos_resp = self.ex.get_positions(symbol)
                if pos_resp and "result" in pos_resp:
                    api_positions = pos_resp["result"].get("list", [])
                    for p in api_positions:
                        if p.get("symbol") == symbol and float(p.get("size", 0) or 0) > 0:
                            return False, f"position_already_exists(symbol={symbol})", 1.0
            except Exception as e:
                logger.warning(f"[ALLOWED] Failed to check Bybit API for position_exists: {e}")
        ...
        for p in open_positions:
            if p.get("symbol") == symbol and float(p.get("size", 0) or 0) > 0:
                unreal = float(p.get("unrealisedPnl", p.get("unrealizedPnl", 0.0)) or 0.0)
                if unreal < 0:
                    return False, f"negative_unreal_pnl(unreal={unreal:.2f})", 1.0
        # V8: Check persistent blacklist from file
        if symbol in self._blacklist:
            return False, f"blacklisted(symbol={symbol})", 1.0
        ...
        # V8: Winrate-based blacklist - block symbols with <50% winrate AND negative avg_pnl after 10+ trades
        # V10: research bots set disable_winrate_filter=True to collect fresh data without inheriting v9 bias.
        if not getattr(self.prod, "disable_winrate_filter", False):
            stats = self._stats_by_symbol.get(symbol, {})
            total_trades = stats.get("total_trades", 0)
            winrate = stats.get("winrate", 0.0)
            avg_pnl = stats.get("avg_pnl", 0.0)
            if total_trades >= 10 and winrate < 0.50 and avg_pnl < 0:
    ...
    async def run(self):
        ...
        while True:
            ...
            try:
                ...
                now = time.time()
                # v8: periodically reload symbol stats (every 5 minutes)
                if now - self._last_stats_update > self._stats_update_interval_sec:
                    self._update_symbol_stats()
                    self._last_stats_update = now
                    # Log wallet balance every 5 minutes
                    try:
                        wallet = self.ex.get_wallet_balance()
                        if wallet.get("retCode") == 0:
                            result = wallet.get("result", {}).get("list", [{}])[0]
                            equity = float(result.get("totalEquity", 0))
                            available = float(result.get("totalAvailableBalance", 0))
                            unreal = float(result.get("totalPerpUPL", 0))
                            logger.info(f"[BALANCE] equity={equity:.2f} available={available:.2f} unreal_pnl={unreal:.2f}")
                    except Exception as e:
                        logger.debug(f"[BALANCE] logging failed: {e}")
                ...
                for sig in evaluated:
                    try:
                        ...
                        # v10 hook: subclass logs every signal to signals_all.jsonl + opens shadow trades.
                        try:
                            self._v10_hook_signal_evaluated(sig, ok, reason, size_mult)
                        except Exception as _e_v10:
                            logger.debug(f"[V10 HOOK] signal_evaluated failed: {_e_v10}")
                        if not ok:
