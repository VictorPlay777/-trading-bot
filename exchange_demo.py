...
class Exchange:
    ...
    def _get_symbol_rules(self, symbol: str):
        ...
        max_order_qty = lot.get("maxOrderQty", "1000000000")
        # maxMktOrderQty is the limit for market orders (typically < maxOrderQty on Bybit).
        max_mkt_qty = lot.get("maxMktOrderQty", max_order_qty)
        rules = {
            "qty_step": Decimal(str(lot.get("qtyStep", "0.001"))),
            "min_qty": Decimal(str(lot.get("minOrderQty", "0.001"))),
            "max_qty": Decimal(str(max_order_qty)),
            "max_mkt_qty": Decimal(str(max_mkt_qty)),
        }
    ...
    def normalize_qty(self, symbol: str, qty, price: float = None, qty_in_notional: bool = False, is_market: bool = False):
        ...
        - is_market=True uses stricter maxMktOrderQty; is_market=False uses maxOrderQty.
        ...
        try:
            ...
            step = rules["qty_step"]
            min_q = rules["min_qty"]
            # Pick the correct exchange limit based on order type.
            ...
            self.logger.info(
                ...
                f"step={step} min_qty={min_q} max_qty={max_q} is_market={is_market} "
                f"price={price} qty_in_notional={qty_in_notional}"
            )
