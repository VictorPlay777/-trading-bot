...
# ── Config ──────────────────────────────────────────────
TOP_N = 9999               # Scan ALL volatile coins (no limit)
POSITION_SIZE_USDT = 50000.0  # USDT per position
MAX_POSITIONS = 9999          # Unlimited concurrent positions
LEVERAGE = 1
TP_PERCENT = 0.005       # 0.5% trailing TP (faster profit taking)
SL_PERCENT = -0.01       # 1% stop loss (wider)
BREAKEVEN_PERCENT = 0.003  # 0.3% to move SL to breakeven
PROB_THRESHOLD = 0.68
HORIZON_BARS = 6
SCAN_INTERVAL = 3600      # Scan tickers every hour (matches 1h timeframe)
...
def _check_tpsl(ex, tracker, logger, predictions=None):
    """Check TP/SL for all open positions. Can be called from anywhere.
    ...
    try:
        ...
        if pos_data.get('retCode') == 0:
            ...
            for pos in positions:
                ...
                # Check breakeven
                if not tracker.is_breakeven(sym) and unrealised_pnl >= breakeven_threshold:
                    tracker.set_breakeven(sym)
                    logger.info(f"Breakeven triggered: {sym} PnL={unrealised_pnl:.2f}")
                
                # Check TP - close immediately when threshold reached
                if unrealised_pnl >= tp_threshold:
                    logger.info(f"TP triggered: {sym} PnL={unrealised_pnl:.2f} >= {tp_threshold:.2f}")
                    ...
                    try:
                        if close_side == "Sell":
                            ...
                            ex.market_buy_symbol(sym, size)
                        logger.info(f"Closed {sym} on TP with profit!")
                        tracker.reset(sym)
                    except Exception as e:
                        logger.error(f"Failed to close {sym} on TP: {e}")
    ...
    while True:
        ...
        if now - last_scan_time >= SCAN_INTERVAL:
            ...
            for sym in top_symbols:
                ...
                # Check TP/SL every 3 symbols for quick reaction (104 positions need frequent checks)
                if sym_count % 3 == 0:
                    ...
                    if sym not in last_candle_ts or last_candle_ts[sym] != latest_ts:
                        ...
                        if signal:
                            ...
                            if not has_position:
                                ...
                                try:
                                    if signal == 'long':
                                        result = ex.market_buy_symbol(sym, qty)
                                    else:
                                        result = ex.market_sell_symbol(sym, qty)
                                    
                                    # Success
                                    qty_str = str(qty)
                                    actual_value = float(qty_decimal) * current_price
                                    logger.info(f"  >> OPENED {signal} {sym} qty={qty_str} value={actual_value:.2f} USDT")
                                    
                                    # Set TP order on exchange immediately
                                    try:
                                        if signal == 'long':
                                            ...
                                            tp_price = current_price * (1 - TP_PERCENT)
                                            side_str = 'Sell'
                                        ex.set_take_profit(sym, side_str, qty, tp_price)
                                        logger.info(f"  >> TP ORDER SET for {sym} @ {tp_price:.4f}")
                                    except Exception as e:
                                        logger.error(f"  >> FAILED to set TP for {sym}: {e}")
                                                ...
                                                try:
                                                    ...
                                                    try:
                                                        if signal == 'long':
                                                            ...
                                                            side_str = 'Sell'
                                                        ex.set_take_profit(sym, side_str, qty, tp_price)
                                                        logger.info(f"  >> TP ORDER SET for {sym} @ {tp_price:.4f} (retry)")
                                                    except Exception as e:
                                                        logger.error(f"  >> FAILED to set TP for {sym}: {e}")
        ...
        # ── Step 4: Check TP/SL for ALL open positions ────
        _check_tpsl(ex, tracker, logger, LATEST_PREDICTIONS)
